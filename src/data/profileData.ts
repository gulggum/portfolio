export const profileData = {
  ...
  strengths: [
    ...
    "복잡한 배포/인증 문제를 끝까지 해결하는 집요한 문제 해결력",
    "처음 겪는 에러라도 원인을 추적해 해결하는 실행력",
  ],
  ...
  devStyle: {
    title: "바이브코딩 + 원리 이해",
    description:
      "Claude AI와 함께 코드를 작성하지만, 단순히 복붙하지 않습니다. 왜 이 구조인지, 어떤 흐름인지 이해한 뒤 적용합니다. 실제 배포 과정에서 발생한 키 인증, 빌드 오류 등을 직접 해결하며, 문제를 끝까지 파고드는 학습 방식을 가지고 있습니다.",
    examples: [
      ...
      "Google Play 배포 과정에서 키 인증 오류를 직접 해결",
      "빌드 환경 문제(경로, Compose 충돌 등)를 단계적으로 해결",
    ],
  },

  // 💬 면접용 메시지
  candidateMessage: `안녕하세요, 사용자 경험을 중심으로 개발하는 프론트엔드 개발자 이희연입니다.

저는 단순히 기능이 동작하는 코드가 아니라, 왜 이렇게 동작하는지 이해하고 설계하는 것을 중요하게 생각합니다. HTML/CSS부터 시작해 JavaScript, React, TypeScript까지 단계적으로 학습하며 기초를 쌓았고, Supabase를 활용해 로그인, 데이터 저장, RLS 권한 설정 등을 직접 적용해보며 프론트엔드에서 백엔드까지 연결되는 흐름을 경험했습니다. 아직 백엔드에 대해 깊이 있는 수준은 아니지만, 실제 프로젝트를 통해 구조를 이해하고 점차 확장해 나가고 있습니다.

대표 프로젝트로는 어린이를 위한 경제 교육 앱 ‘키즈스톡’을 제작했습니다. 이 프로젝트에서는 단순한 기능 구현을 넘어서, 실제 서비스처럼 동작하도록 설계하고 배포까지 진행했습니다. 특히 Google Play 배포 과정에서 업로드 키 문제, SHA1 불일치 오류, 빌드 환경 문제 등을 직접 겪었고, keystore와 인증 구조를 이해하며 끝까지 해결한 경험이 있습니다.

이 과정에서 저는 단순히 코드를 작성하는 것을 넘어, 문제의 원인을 추적하고 해결하는 개발자로 한 단계 성장했다고 느꼈습니다.

또한 AI 포트폴리오 프로젝트를 통해 Claude API를 실제 서비스에 연결하고, 사용자와 상호작용하는 인터페이스를 구현했습니다. AI를 단순히 사용하는 것이 아니라, 구조를 이해하고 서비스에 맞게 활용하는 것이 중요하다고 생각합니다.

저는 새로운 기술을 두려워하지 않고, 막히는 문제를 끝까지 해결하는 끈기를 가진 개발자입니다. 앞으로도 사용자 입장에서 생각하며, 더 나은 경험을 만드는 개발자로 성장하고 싶습니다.
`,
};
